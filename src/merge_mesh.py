...
class Mesher(object):
    ...
    def __init__(self, cfg, args, eslam, points_batch_size=160000, ray_batch_size=100000):
    ...
    def blend_ret(self, pnts, ret_list, world2rf_point, power=2):
        ...
        print(world2rf_point_expanded.shape)
    ...
    def get_mesh(self, mesh_out_file, all_planes_list, decoders_list, world2rf, keyframe_dict,
        ...
        """
        print('submap fusion', datetime.datetime.now())
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Arguments for submap fusion.'
    )
    ...
    #cfg = config.load_config('/data0/sgl/ESLAM-main/configs/Replica/room0_dis.yaml')
    cfg = config.load_config(args.config, '/data0/sgl/ESLAM-main/configs/PLGSLAM.yaml')
    mesh_out_file = '/data0/sgl/ESLAM-main/output/Replica/room0_dis/mesh_fusion.ply'

    eslam = ESLAM(cfg, args)
    mesh = Mesher(cfg, args, eslam)
    model = config.get_model(cfg)
    model = model.to("cpu")
    model.bound = eslam.bound
    #output = cfg['data']['output'] if args.output is None else args.output
    #ckptsdir = f'{output}/ckpts'


    ckptsdirs = [
        '/data0/sgl/ESLAM-main/output/Replica/room0_dis/0505_1/ckpts',
        '/data0/sgl/ESLAM-main/output/Replica/room0_dis/0505_2/ckpts',
        '/data0/sgl/ESLAM-main/output/Replica/room0_dis/0505_3/ckpts',
        '/data0/sgl/ESLAM-main/output/Replica/room0_dis/0505_4/ckpts'
    ]
    ckptsdirs = [
        '/data0/sgl/ESLAM-main/output/Replica/room0_dis/0505_1/ckpts'
    ]
    ...
    for ckptsdir in ckptsdirs:
        if os.path.exists(ckptsdir):
            ...
            if len(ckpts) > 0:
                ...
                all_planes_list.append(ckpt['all_planes'])
                decoder_state_dict = ckpt['decoder_state_dict']
                model.load_state_dict(decoder_state_dict)
                decoders_list.append(model)
                keyframe_dict.extend(ckpt['keyframe_dict'])
    ...
    world2rf_paramlist = [[3.452987416406734678e+00, 4.546110134135947223e-01, 5.936285447159415085e-01],
                          [2.644848971900370493e+00, 1.363642205122169937e+00, 2.394058811711397838e-01],
                          [1.687581608494627794e+00, 8.627709956980221806e-01, 3.030689912211013692e-01],
                          [3.129274400075018114e+00, 2.014046039907980479e+00, 6.377345007005607025e-01]]
    world2rf_paramlist = [[3.452987416406734678e+00, 4.546110134135947223e-01, 5.936285447159415085e-01]]

    mesh.get_mesh(mesh_out_file, all_planes_list, decoders_list, world2rf_paramlist, keyframe_dict,
                 device='cuda:0')
