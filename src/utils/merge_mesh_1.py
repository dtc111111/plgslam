...
class M_Mesher(object):
    ...
    def eval_points(self, p, all_planes_list, decoders_list, world2rf):
        ...
        for pi in p_split:
            ...
            for ckptsdir, all_planes in zip(ckptsdirs, all_planes_list):
                if os.path.exists(ckptsdir):
                    ...
                    if len(ckpts) > 0:
                        ...
                        decoders.load_state_dict(decoder_state_dict)
    ...
    def blend_ret(self, pnts, ret_list, world2rf_point, power=2):
        ...
        world2rf_point_expanded = world2rf_point.unsqueeze(0).to(pnts_expanded.device)
        ...
        condition = (ret_list_stacked_permute[:, :, 0] == 1) | (ret_list_stacked_permute[:, :, 0] == -1)
        weights_permute = weights.unsqueeze(2)
        weights_permute[condition] = 0
        weighted_values = weights_permute * ret_list_stacked_permute  # [500000, 3, 1] [500000,3, 4]
    ...
    def get_mesh(self,  decoders_list,
        ...
        mesh_out_file = '/data0/sgl/ESLAM-main/output/Replica/room0_dis/mesh_fusion_1.ply'
        ...
        all_planes_list = [tuple([plane.to(self.device) for plane in planes] for planes in all_planes) for all_planes in
                           all_planes_list]
        world2rf = torch.tensor([[3.452987416406734678e+00, -4.546110134135947223e-01, -5.936285447159415085e-01],
