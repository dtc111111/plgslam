...
if __name__ == '__main__':
    ...
    # checkpoint_filename = 'final_ckpt.pth'
    # ckptsdir = f'{output}/{checkpoint_filename}'
    # ckpt = torch.load(ckptsdir)

    pose = np.load('/data0/wjy/sgl/vox_fusion/apartment/1107/frame_poses.npy')
    ...
    #estimate_c2w_dict = poses_est
    N = gt_ckpt['idx']-1

    #pose = load_poses(N, '/data0/wjy/sgl/vox_fusion/apartment/1107/trajectory.txt')
    estimate_c2w_list = torch.from_numpy(pose)

    #estimate_c2w_list = torch.stack([estimate_c2w_dict[i] for i in range(N + 1)], dim=0)
    gt_c2w_list = gt_c2w_list[:-1, :, :]
    poses_gt, mask = convert_poses(gt_c2w_list, N, scale)
    #estimate_c2w_list = estimate_c2w_list[:-1, :, :]
