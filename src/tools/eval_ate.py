...
def evaluate_ate(first_list, second_list, output, plot="", _args=""):
    # parse command line
    ...
    args.save_associations = f'{output}/poses_associations.txt'
    if args.save_associations:
        ...
        file.close()
    args.save = f'{output}/poses_est.txt'
    if args.save:
...
def evaluate(poses_gt, poses_est, output, plot):
    ...
    results = evaluate_ate(poses_gt, poses_est, output, plot)
    print(results)
...
if __name__ == '__main__':
    ...
    """
    ckpt = torch.load('/data0/wjy/sgl/ESLAM-main/output/Replica/part2_0929/ckpts/01099.tar', map_location=torch.device('cpu'))
    c2w = ckpt['estimate_c2w_list']
    gt_c2w = ckpt['gt_c2w_list']
    print(c2w[0], gt_c2w[0])
    parser = argparse.ArgumentParser(
    ...
    #cfg = config.load_config(args.config, '/data0/sgl/ESLAM-main/configs/PLGSLAM.yaml')
    #cfg = config.load_config(args.config, '/data0/wjy/sgl/PLGSLAM/configs/PLGSLAM.yaml')
    scale = cfg['scale']
    ...
    if os.path.exists(ckptsdir):
        ...
        if len(ckpts) > 0:
            ...
            ###### if trained without gtpose######
            #gt_ckpt_path = '/data0/wjy/sgl/PLGSLAM/output/Apartment/1026_lr_t/ckpts/12594.tar'  # 8521
            #gt_ckpt_path = '/data0/sgl/ESLAM-main/output/Apartment/23/1016_local_max4/ckpts/12594.tar'#6001
            #gt_ckpt_path = '/data0/sgl/ESLAM-main/output/Apartment/23/1024_m_lr/ckpts/12594.tar' #9988
            # gt_ckpt_path = '/data0/wjy/sgl/nice-slam/output/Replica/room0/ckpts/01999.tar'
            #gt_ckpt = torch.load(gt_ckpt_path, map_location=torch.device('cpu'))
            #gt_c2w_list = gt_ckpt['gt_c2w_list']

            ###### if trained withgtpose######
            ...
            ############ keyframe_eval ##### txt:  timestamp tx ty tz qx qy qz qw ############
            # with open('/data0/sgl/ESLAM-main/KeyFrameTrajectory.txt', 'r') as file:
            #     lines = file.readlines()
            # poses_est = torch.tensor([list(map(float, line.strip().split()[1:])) for line in lines])
            # poses_gt_full, mask = convert_poses(gt_c2w_list, N, scale)
            # indices_from_file = [float(line.strip().split()[0]) for line in lines]
            # selected_indices = torch.tensor(indices_from_file)
            # poses_gt = poses_gt_full[selected_indices.long()]

            ########### write poses_gt to a text file  timestamp tx ty tz qx qy qz qw
            # Combine indices and poses_gt into a tensor with eight columns
            # combined_tensor = torch.cat((selected_indices.view(-1, 1), poses_gt), dim=1)
            # Convert the tensor to a list of strings
            # lines_to_write = [f"{selected_indices[i]:.6f} {' '.join(map(str, poses_gt[i].tolist()))}\n" for i in
            #                   range(len(selected_indices))]
            # Write the lines to a text file
            # with open('/data0/sgl/ESLAM-main/chess_keyframe_poses_gt.txt', 'w') as file:
            #     file.writelines(lines_to_write)

            ############tum
            # pose_folder = "/data0/dataset/7scenes/chess/seq-01"
            # groundtruth_file = "/data0/dataset/7scenes/chess_groundtruth.txt"
            # # 获取pose文件列表并排序
            # pose_files = sorted([filename for filename in os.listdir(pose_folder) if filename.endswith(".pose.txt")])
            # # 生成groundtruth.txt
            # with open(groundtruth_file, 'w') as groundtruth:
            #     for pose_filename in pose_files:
            #         frame_number = pose_filename.split('.')[0].split('-')[-1].lstrip('0') or '0'
            #
            #         with open(os.path.join(pose_folder, pose_filename), 'r') as pose_file:
            ...
            #         # 将四元数和平移向量写入groundtruth.txt
            #         groundtruth.write(
            #             f"{frame_number}{' '.join(map(str, translation_vector))} {' '.join(map(str, rotation_quaternion))}\n")
            # print('success')


            evaluate(poses_gt, poses_est, output, plot=f'{output}/eval_ate_plot.png')