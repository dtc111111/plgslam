...
def evaluate_ate(first_list, second_list, output,  plot="", _args=""):
    ...
    #
    # with open(f'{output}/poses_est.txt', 'w') as file:
    #     for pose in second_xyz_full_aligned:
    #         file.write(f'{pose}\n')
...
def convert_poses_for_ap_gt(c2w_list, N, scale, gt=True):
    poses = []
    mask = torch.ones(N + 1).bool()
    for idx in range(0, N + 1):
        if gt:
        ...
        # trans = torch.tensor([[-0.14031718, -0.875229, -0.46290958, 0.75258389],
        #                       [0.217254, -0.48335774, 0.84803655, 0.32966271],
        #                       [-0.96597712, 0.01842514, 0.2579704, 3.28585226],
        #                       [0., 0., 0., 1.]])
        # #print(trans.dtype, c2w_list[idx].dtype)
        # c2w_list[idx] = trans @ c2w_list[idx]
...
if __name__ == '__main__':
    ...
    checkpoint_filename = 'checkpoint12594.pt'
    #checkpoint_filename = '12594.tar'
    ...
    if os.path.exists(ckptsdir):
        ...
        #          for f in sorted(os.listdir(ckptsdir)) if 'tar' in f]
            ...
            #gt_ckpt_path = '/data0/wjy/sgl/PLGSLAM/output/scannet/scene0207_00/01987.tar'#change!
            #gt_ckpt_path = '/data0/wjy/sgl/PLGSLAM/output/Apartment/1026_lr_t/ckpts/12594.tar'  # change!
            gt_ckpt_path = '/data0/sgl/ESLAM-main_original_version/output/Apartment/0820/ckpts/12594.tar'  # 6001
            ...
            poses_gt, mask = convert_poses_for_ap_gt(gt_c2w_list, N, scale)
            poses_est, _ = convert_poses(estimate_c2w_list, N, scale)
            poses_est = poses_est[mask]



                # 保存 poses_gt 和 poses_est 为文本文件
            # with open(f'{output}/poses_gt.txt', 'w') as file:
