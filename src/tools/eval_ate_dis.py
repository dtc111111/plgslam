...
def evaluate_ate(first_list, second_list, output, plot="", _args=""):
    ...
    args.save_associations = f'{output}/poses_associations_0.txt'
    if args.save_associations:
        ...
        file.close()
    args.save = f'{output}/poses_est_0.txt'
    if args.save:
...
if __name__ == '__main__':
    ...
    output = '/data0/sgl/ESLAM-main/output/Replica/room0_dis'
    ckpt_path = output + '/0425_0/ckpts/00499.tar'
    ckpt = torch.load(ckpt_path, map_location=torch.device('cpu'))
    estimate_c2w_list = ckpt['estimate_c2w_list']

    # ckpt_path_2 = '/data0/sgl/ESLAM-main/output/Replica/room0_dis/0425_500/ckpts/00499.tar'
    # ckpt = torch.load(ckpt_path_2, map_location=torch.device('cpu'))
    # estimate_c2w_list = torch.cat([estimate_c2w_list, ckpt['estimate_c2w_list']], dim=0)
    #
    # ckpt_path_3 = '/data0/sgl/ESLAM-main/output/Replica/room0_dis/0425_1000/ckpts/00499.tar'
    # ckpt = torch.load(ckpt_path_3, map_location=torch.device('cpu'))
    # estimate_c2w_list = torch.cat([estimate_c2w_list, ckpt['estimate_c2w_list']], dim=0)
    #
    # ckpt_path_4 = '/data0/sgl/ESLAM-main/output/Replica/room0_dis/0425_1500/ckpts/00499.tar'
    # ckpt = torch.load(ckpt_path_4, map_location=torch.device('cpu'))
    # estimate_c2w_list = torch.cat([estimate_c2w_list, ckpt['estimate_c2w_list']], dim=0)

    ###### if trained without gtpose######
    gt_ckpt_path = '/data0/sgl/ESLAM-main/output/Replica/room0/ckpts/01999.tar'

    gt_ckpt = torch.load(gt_ckpt_path, map_location=torch.device('cpu'))
    gt_c2w_list = gt_ckpt['gt_c2w_list'][:500]
    ...
    evaluate(poses_gt, poses_est, output, plot=f'{output}/eval_ate_plot_0.png')